...
class MongoComparator {
  ...
  static int compareWithKeySelector(dynamic a, dynamic b, Map keySelector){
    ...
    valueForNestedKey(dynamic data, String key) {
      return key.split('.').fold(data, (m, k) => m[k]);
    }

    for (String key in keySelector.keys) {
      order = keySelector[key];
      int c = compare(valueForNestedKey(a, key), valueForNestedKey(b, key));
